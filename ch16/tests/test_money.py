...
class TestMoney:
    ...
    def test_sum_plus_money(self):
        ...
        bank = Bank()
        bank.add_rate("CHF", "USD", 2)
        sum_val = SumVal(five_bucks, ten_francs).plus(five_bucks)
        result = bank.reduce(sum_val, "USD")
    ...
    def test_sum_times(self):
        ...
        bank = Bank()
        bank.add_rate("CHF", "USD", 2)
        sum_val = SumVal(five_bucks, ten_francs).times(2)
        result = bank.reduce(sum_val, "USD")
        assert Money.dollar(20) == result

    # def test_plus_same_currency_returns_money(self):
    #     sum_val = Money.dollar(1).plus(Money.dollar(1))
    #     assert isinstance(sum_val, Money)
